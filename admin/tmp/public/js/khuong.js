...
var ml=$("#code").val();
  if(ml==""){
    $('#code').attr('value',makeid());
  }
$(document).ready(function(){
    $("[name=discount]").keyup(function(vaidai){

    var vaidai = $(vaidai.currentTarget).val();
    if (vaidai==="") {
      $("#from, #to").attr("required",false)
  ...
  $("[name=price]").keyup(function(hellobaybe) {
    var hellobaybe = $(hellobaybe.currentTarget).val();
    if (hellobaybe==="") {
      $("[name=discount]").val("")
      $("[name=discount]").attr("disabled",true)
    }else{
      $("[name=discount]").attr("disabled",false)
      $("[name=discount_price]").attr("disabled",false)
    }
  });
