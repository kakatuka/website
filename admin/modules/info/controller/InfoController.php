...
class InfoController extends Controller {
	...
	public function edit() {
		...
		if (isset($_GET['id']) && is_numeric($_GET['id'])) {
			if ($this->modelInfo->checkId($_GET['id']) == FALSE) {
				$this->view->data['data'] = $this->modelInfo->getUserById($_GET['id']);
	...
	public function save() {
		if (isset($_POST['submit'])) {
			...
			if (isset($_POST['id_posts']) && is_numeric($_POST['id_posts'])) {
				...
				$this->modelInfo->update($data, $_POST['id_posts']);
				$mess = array(
				...
				$this->modelInfo->insertData($data);
				$mess = array(
			...
			if ($_POST['submit'] == 'save') {
				...
				redirect(base_url() . 'info/info/add');
			}
		}
	}
	public function del() {
	...
	public function dellAll() {
		if (isset($_POST['all'])) {
			if (!empty($_POST['all']) && is_array($_POST['all'])) {
				$names_id = $_POST['all'];
				$this->modelInfo->dellWhereInArray($names_id);
				$mess = array(
		...
		if (isset($_POST['status'])) {
			if (!empty($_POST['all']) && is_array($_POST['all'])) {
				...
				foreach ($names_id as $value) {
					$this->modelInfo->update($data, $value);
				}
				$mess = array(
	...
	public function uploadStatus() {
		if (isset($_POST['status']) && isset($_POST['id'])) {
			...
			$this->modelInfo->statusNew($id, $data);
		}
	}
}