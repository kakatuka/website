...
$('body').on('dblclick', '#back_folder', function(event) {
	...
	$('body').on('click', '#modelDeleteAll #agree_del_all', function(event) {
		...
		if (allVals.length >= 1) {
			$.ajax({
				url: baseUrl+'slider/slider/dellAll',
				type: 'POST',
				dataType: 'json',
				data: {all: allVals},
			})
	...
	$('body').on('click', '#lock_user', function(event) {
		...
		if (allVals.length >= 1) {
			var status = 'public';
			$.ajax({
				url: baseUrl+'slider/slider/status',
				type: 'POST',
				dataType: 'json',
	...
	$('body').on('click', '#lock_categories', function(event) {
		...
		if (allVals.length >= 1) {
			var status = 'public';
			$.ajax({
				url: baseUrl+'slider/categories/status',
				type: 'POST',
				dataType: 'json',
				data: {status: status,all:allVals},
			})
	...
	$('body').on('click', '#modelDeleteAll #agree_del_all_categories', function(event) {
		...
		if (allVals.length >= 1) {
			$.ajax({
				url: baseUrl+'slider/categories/dellAll',
				type: 'POST',
				dataType: 'json',
				data: {all: allVals},
			})
	...
	$('body').on('click', '.search_button_categories', function(event) {
		...
		if (search=="") {
			window.location.assign(baseUrl+'slider/categories/index');
		}else{
			window.location.assign(baseUrl+'slider/categories/index&s='+search);
		}
...
$('body').on('click', '.search_button_users', function(event) {
	...
	if (search=="") {
		window.location.assign(baseUrl+'slider/slider/index');
	}else{
		window.location.assign(baseUrl+'slider/slider/index&s='+search);
	}
