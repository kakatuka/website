...
$('body').on('dblclick', '#back_folder', function(event) {
    ...
    $('body').on('click', '#modelDeleteAll #agree_del_all', function(event) {
        ...
        if (allVals.length >= 1) {
            $.ajax({
                url: baseUrl+'video/video/dellAll',
                type: 'POST',
                dataType: 'json',
                data: {all: allVals},
            })
    ...
    $('body').on('click', '#lock_user', function(event) {
        ...
        if (allVals.length >= 1) {
            var status = 'public';
            $.ajax({
                url: baseUrl+'video/video/status',
                type: 'POST',
                dataType: 'json',
...
$('body').on('click', '.search_button_users', function(event) {
    ...
    if (search=="") {
        window.location.assign(baseUrl+'video/video/index');
    }else{
        window.location.assign(baseUrl+'video/video/index&s='+search);
    }
