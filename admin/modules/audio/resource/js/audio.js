...
$('body').on('dblclick', '#myModalFile .media-col img.img-folder-media', function(event) {
	...
	$.ajax({
		url: baseUrlcloud+'settings/settings/openDirectoryFile',
		type: 'POST',
		dataType: 'json',
	...
	.done(function(data) {
		...
		if (data.status==true) {
			$('#myModalFile').find('.modal-body-file').fadeOut(100, function(){
                $('#myModalFile').find('.modal-body-file').html(data.html).fadeIn();
            });
		}
	});
});
$('body').on('dblclick', '#back_folder_file', function(event) {
	event.preventDefault();
	var current_folder = $(this).parent('.modal-body-file');
	var dir = $('#directory').val();
	$.ajax({
		url: baseUrlcloud+'settings/settings/backDirectoryFile',
		type: 'POST',
		dataType: 'json',
		data: {back: 'true',dir:dir},
	})
	.done(function(data) {
		if (data.status==true) {
...
$(document).ready(function() {
    $.ajax({
        url: baseUrlcloud+'media/Uploadfile/openOneFilesDirectory',
        type: 'POST',
        dataType: 'json',
        data: {
            open: security_key
    ...
    .done(function(data) {
        $( "#loadMediaModelFile" ).append(data.html);
    });
});
...
$('body').on('click', '.choose_file', function(event) {
		...
		if (list.length == 0) {
			...
			var src = list[0];
			if (body_file.attr('data-title')=='thumbnail_file') {
				var title = 'thumbnail_file';
			}else{
				var title = '';
			}
			/*$.ajax({
			...
			$('.modal-file-choose').find('.hidden_thumb_file').val(src);
			$(this).parents('.modal-file-choose').find('.load-img').attr('src', baseUrlcloud+'timthumb.php?src='+baseUrlcloud+'tmp/public/images/img-mp3.png&h=150&w=210&zc=2');
	...
	$('body').on('click', '.del-file-choose-pages', function(event) {
		event.preventDefault();
		$(this).parents('.modal-file-choose').find('.hidden_thumb_file').val('');
		$(this).parents('.modal-file-choose').find('.file-website').attr('src', baseUrlcloud+'timthumb.php?src='+baseUrl+'tmp/public/images/img.png&h=150&w=210&zc=2');
	});
// Ajax chọn file trong Modal
	...
	.done(function(data) {
		...
		if (data.status==true) {
			$('#myModalFile').find('.modal-body').fadeOut(100, function(){
				$('#myModalFile').find('.modal-body').html(data.html123).fadeIn();
			}); 
			toastr["success"](data.mess);
...
$('body').on('dblclick', '#back_folder', function(event) {
	...
	$('body').on('click', '#modelDeleteAll #agree_del_all', function(event) {
		...
		if (allVals.length >= 1) {
			$.ajax({
				url: baseUrl+'audio/audio/dellAll',
				type: 'POST',
				dataType: 'json',
				data: {all: allVals},
			})
	...
	$('body').on('click', '#lock_user', function(event) {
		...
		if (allVals.length >= 1) {
			var status = 'public';
			$.ajax({
				url: baseUrl+'audio/audio/status',
				type: 'POST',
				dataType: 'json',
...
$('body').on('click', '.search_button_users', function(event) {
	...
	if (search=="") {
		window.location.assign(baseUrl+'audio/audio/index');
	}else{
		window.location.assign(baseUrl+'audio/audio/index&s='+search);
	}
